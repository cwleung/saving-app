...
import {
  ...
  Pencil, ChevronDown, ChevronUp, PiggyBank, RefreshCw, TrendingUp,
} from 'lucide-react';
...
export function PotsPage() {
  ...
  const totalBalance = pots.reduce((s, p) => s + computeBalance(p.id, transactions), 0);

  const totalMonthlyIn = pots.reduce((s, pot) => {
    const inflow = regularSpendings
      .filter((r) => r.potId === pot.id && r.transactionType === 'income')
      .reduce((sum, r) => sum + r.amount * (MONTHLY_FACTOR[r.frequency] ?? 1), 0);
    return s + inflow;
  }, 0);

  function handleAdd(e: React.FormEvent) {
    e.preventDefault();
    const color = POT_COLORS[pots.length % POT_COLORS.length];
    addPot({
  ...
  function handleAction(pot: Pot, currentBalance: number) {
    const amt = parseFloat(actionAmount);
    if (!amt || amt <= 0) return;
    if (actionType === 'withdraw' && amt > currentBalance) return;
    addTransaction({
      id: crypto.randomUUID(),
      type: actionType === 'deposit' ? 'income' : 'expense',
      amount: amt,
      category: actionType === 'deposit' ? 'Pot Deposit' : 'Pot Withdrawal',
      description: `${actionType === 'deposit' ? 'Deposit to' : 'Withdrawal from'} ${pot.name}`,
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-5 pb-28">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Ring-fence money for a purpose</p>
        </div>
        <button
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden">
          ...
          <form onSubmit={handleAdd} className="px-5 py-4 space-y-3">
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 text-xs text-indigo-700 leading-relaxed">
              💡 After creating a pot, go to <strong>Regular</strong> to add a recurring income — select this pot as the destination to automate monthly deposits.
            </div>
            <button
              type="submit"
      ...
      {pots.length === 0 && !showAdd ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1 max-w-xs">
            Create a pot for travel, car maintenance, or any purpose — then add a recurring deposit
        ...
        <div className="space-y-4">
          {pots.map((pot) => {
            const balance = computeBalance(pot.id, transactions);
            const potTxs = transactions
              .filter((t) => t.potId === pot.id)
              .sort((a, b) => b.date.localeCompare(a.date));
            const linkedRecurring = regularSpendings.filter((r) => r.potId === pot.id);
            const monthlyIn = linkedRecurring
              .filter((r) => r.transactionType === 'income')
              .reduce((s, r) => s + r.amount * (MONTHLY_FACTOR[r.frequency] ?? 1), 0);
            const monthlyOut = linkedRecurring
              .filter((r) => r.transactionType === 'expense')
              .reduce((s, r) => s + r.amount * (MONTHLY_FACTOR[r.frequency] ?? 1), 0);
            const isExpanded = expandedPotId === pot.id;
            const isActioning = actionPotId === pot.id;
            const withdrawAmt = parseFloat(actionAmount);
            const overBalance = actionType === 'withdraw' && actionAmount && withdrawAmt > balance;

            return (
              <div
                key={pot.id}
                className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden"
              >
                {/* Top color strip */}
                ...
                <div className="p-5">
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-3xl font-bold mt-1" style={{ color: pot.color }}>
                      {fmt(balance)}
                    </p>
                  </div>
                    ...
                    <div className="flex gap-2 mb-4">
                      ...
                      {monthlyOut > 0 && (
                        <div className="flex-1 bg-red-50 rounded-xl px-3 py-2">
                          <p className="text-[10px] text-red-500 uppercase font-semibold tracking-wide">
                            Out / mo
                          </p>
                          <p className="text-sm font-bold text-red-600 mt-0.5">-{fmt(monthlyOut)}</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {isActioning ? (
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          placeholder="Amount…"
                          autoFocus
                          className="flex-1 bg-gray-50 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:bg-white transition-colors"
                        />
                        <button
                          ...
                          className="bg-indigo-500 hover:bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed text-white px-4 py-2.5 rounded-xl text-sm font-semibold cursor-pointer transition-colors"
                        >
                          OK
                        </button>
                        <button
                      ...
                      {overBalance && (
                        <p className="text-xs text-red-500">Amount exceeds pot balance</p>
                      )}
                    </div>
                  ) : (
                    <button
                      onClick={() => {
                        setActionPotId(pot.id);
                    ...
                    >
                      <TrendingUp className="w-4 h-4" /> Deposit / Withdraw
                    </button>
                  )}
                            ...
                            <div key={t.id} className="flex items-center gap-2 text-xs">
                              ...
                              <span
                                className={`font-semibold shrink-0 ${
                                  t.type === 'income' ? 'text-indigo-600' : 'text-red-500'
                                }`}
                              >
                                {t.type === 'income' ? '+' : '-'}
                                {fmt(t.amount)}
